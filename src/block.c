...
void initMatrix(uint8_t **matrix, uint32_t n, uint32_t m){
    for (uint32_t rows = 0; rows < n; rows++)
    {
        *(matrix) = malloc(sizeof(uint8_t) * m);
    }
}
...
uint8_t *verifyBlock(block_t *block, uint32_t *nb_unk){
    uint8_t *s = malloc(sizeof(uint8_t)*((block->size_block)+(block->size_redundance)));
    *nb_unk = 0;
    for (uint32_t i = 0; i < block->size_block + block->size_redundance; i++)
    {
...
void makeLinearSystem(block_t *block, uint8_t *lost_indexes, uint32_t nb_unk, uint8_t **coeffs, uint8_t **A, uint8_t **b){
    for (size_t i = 0; i < nb_unk; i++)
    {
        ...
        *(b+i) = gf_256_mul_vector_ret(*(block->symb_list + block->size_block + i), 1, block->size_symbol);
...
void freeBlock(block_t *block){
    freeMatrix(block->symb_list, block->size_block + block->size_redundance);
    free(block);
}


void printBlock(block_t *block){
    for (int i = 0; i < block->size_block + block->size_redundance; i++)
    {
        printf("%s\n",*(block->symb_list + i));
    }
    printf("\n\n");
...
void processBlock(block_t **list_block, uint8_t size, uint32_t seed){
    for (uint8_t i = 0; i < size; i++)
    {
        ...
        uint8_t **coeffs = gen_coefs(prng, (*(list_block + i))->size_redundance, (*(list_block + i))->size_block);

        if (nb_unk != 0){
            printf("Number Unknown : %d\n", nb_unk);
            uint8_t **A = malloc(sizeof(uint8_t *) * nb_unk);
            uint8_t **b = malloc(sizeof(uint8_t *) * nb_unk);
            makeLinearSystem(*(list_block + i), s, nb_unk, coeffs, A, b);
        ...
        free(s);
        freeMatrix(coeffs, (*(list_block + i))->size_redundance);
    }
